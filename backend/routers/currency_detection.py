...
async def scan_currency(file: UploadFile = File(...)):
    if not file.content_type or not file.content_type.startswith("image/"):
        # Fallback check on file extension if content_type is missing
        if not file.filename or not file.filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp', '.avif')):
    ...
    try:
        ...
        print(f"Currency scan fallback: {e}")
